        ...
        for symbol in symbols:
            try:
                ...
                if not df.empty:
                    ...
                    # Filter to required columns and check data quality
                    df = df[required_columns]
                    
                    # Check for missing data
                    missing_data = df.isnull().any(axis=1).sum()
                    if missing_data > 0:
                        missing_pct = (missing_data / len(df)) * 100
                        logger.warning(f"{symbol}: {missing_data} rows ({missing_pct:.2f}%) have missing data")
            ...
            try:
                ...
                for inst in instruments:
                    try:
                        ...
                        # Check if expiry is within date range
