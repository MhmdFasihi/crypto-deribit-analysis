...
class VolatilityOptionsAnalysisSystem:
    ...
    def generate_visualizations(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        try:
            # Check if we have any valid results to visualize
            has_volatility_results = any(not df.empty for df in self.volatility_results.values()) if self.volatility_results else False
            has_options_results = any(bool(result) for result in self.options_results.values()) if self.options_results else False
            
            if not has_volatility_results and not has_options_results:
                logger.warning("No valid analysis results available for visualization")
                # Return empty results instead of raising an error
                return {"warning": "No valid analysis results available for visualization"}
    ...
    def generate_report(self) -> str:
        ...
        Returns:
            Path to the generated report or message about missing data
        """
        try:
            # Check if we have any valid results to include in the report
            has_volatility_results = any(not df.empty for df in self.volatility_results.values()) if self.volatility_results else False
            has_options_results = any(bool(result) for result in self.options_results.values()) if self.options_results else False
            
            if not has_volatility_results and not has_options_results:
                logger.warning("No valid analysis results available for report generation")
                
                # Generate a simple report with the analysis metadata
                ...
                # Create a simple report indicating no data was found
                report_path = self.output_dir / "analysis_report.html"
                with open(report_path, 'w') as f:
                    f.write(f"""
                    <html>
                    <head>
                        <title>Crypto Analysis Report - No Data</title>
                        <style>
                    ...
                    <body>
                        ...
                        <div class="warning">
                            ...
                            <p>The analysis did not find any valid data for the specified symbols and date range:</p>
                            <ul>
                                <li><strong>Symbols:</strong> {', '.join(self.symbols)}</li>
                                <li><strong>Date Range:</strong> {self.start_date} to {self.end_date}</li>
                            </ul>
                        </div>
                        <div class="info">
                            ...
                            <ul>
                                ...
                                <li>Missing API credentials for authenticated endpoints</li>
                            </ul>
                            <h2>Suggestions:</h2>
                            <ul>
                                <li>Check your internet connection</li>
                                <li>Try a different date range</li>
                                <li>Verify your API credentials in the .env file</li>
                                <li>Try using the test environment with --test-env flag</li>
                            </ul>
                        </div>
                        <div class="info">
                ...
                logger.info(f"Generated basic report with no data notification: {report_path}")
            ...
            if self.volatility_analyzer and self.volatility_results:
                if has_volatility_results:
                    try:
                        price_summary = self.volatility_analyzer.get_summary_statistics()
                    except Exception as e:
                        logger.error(f"Error getting volatility summary: {str(e)}")
                        price_summary = {}
                    
                    # Get anomaly impact analysis for symbols with data
                    anomaly_impact = {}
                    for symbol in self.symbols:
                        if (symbol in self.volatility_results and 
                            not self.volatility_results[symbol].empty):
                            try:
            ...
            if self.options_analyzer and self.options_results and has_options_results:
            ...
            try:
                report_path = self.output_dir / "analysis_error_report.html"
                with open(report_path, 'w') as f:
                    f.write(f"""
                    <html>
                    <head>
                        <title>Crypto Analysis Error Report</title>
                        <style>
                            body {{ font-family: Arial, sans-serif; margin: 20px; }}
                            h1, h2 {{ color: #333; }}
                            .error {{ color: #f44336; }}
                ...
                logger.error(f"Error creating error report: {str(write_error)}")
    ...
    def run_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            has_price_data = any(not df.empty for df in self.price_data.values()) if self.price_data else False
            has_options_data = any(not df.empty for df in self.options_data.values()) if self.options_data else False
            ...
            # Step 4: Generate visualizations (continue even if no data)
            ...
            # Step 5: Generate report (continue even if no data)
            logger.info("Step 5: Generating report")
            try:
                report_path = self.generate_report()
            except Exception as e:
                logger.warning(f"Could not generate report: {str(e)}")
                report_path = "No report generated"
            ...
            summary = {
                ...
                "results": {
                    "volatility": {
                        "symbols": [
                            symbol for symbol, df in self.volatility_results.items() 
                            if not df.empty
                        ],
                        "count": sum(1 for df in self.volatility_results.values() if not df.empty)
                    },
                    "options": {
                        "symbols": [
                            symbol for symbol, result in self.options_results.items() 
                            if result and 'options_data' in result and not result['options_data'].empty
                        ],
                        "count": sum(
                            1 for result in self.options_results.values() 
                            if result and 'options_data' in result and not result['options_data'].empty
                        )
                    }
                },
            ...
            # Add warning if no data was available
            if not has_price_data and not has_options_data:
                summary["warning"] = "No data was available for the specified symbols and date range"
