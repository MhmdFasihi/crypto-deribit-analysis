...
class CryptoVolatilityOptionsVisualizer:
    ...
    def create_html_report(
        ...
        try:
            ...
            has_price_data = price_summary and any(bool(summary) for summary in price_summary.values())
            has_options_data = options_summary and any(bool(summary) for summary in options_summary.values())
            has_impact_data = anomaly_impact and any(bool(impact) for impact in anomaly_impact.values())
            ...
            <head>
                ...
                <style>
                    ...
                    .warning {{
                        color: {self.color_scheme['negative']};
                        padding: 10px;
                        border: 1px solid {self.color_scheme['negative']};
            ...
            if not has_price_data and not has_options_data and not has_impact_data:
                html_content += """
                <div class="warning">
                    <h2>No Data Available</h2>
                    <p>No valid analysis data is available. This could be due to:</p>
                    <ul>
                        <li>No data found for the specified date range</li>
            ...
            # Add dashboard links section if any dashboards exist
            symbols = set(self.price_results.keys()) | set(self.options_results.keys())
            dashboards_exist = False
            
            for symbol in sorted(symbols):
                dashboard_path = f"{symbol}_dashboard.html"
                if (self.output_dir / dashboard_path).exists():
                    dashboards_exist = True
                    break
            ...
            # Add price analysis summary if available
            if has_price_data:
                html_content += """
            ...
            # Add options analysis summary if available
            if has_options_data:
                html_content += """
                ...
                for symbol, summary in sorted(options_summary.items()):
                    if not summary or 'volume_analysis' not in summary or 'iv_analysis' not in summary:
                        continue
                        
                    vol_analysis = summary.get('volume_analysis', {})
                    iv_analysis = summary.get('iv_analysis', {})
                    
                    if not vol_analysis or not iv_analysis:
                        continue
            ...
            # Add anomaly impact analysis if available
            if has_impact_data:
                html_content += """
            ...
            try:
                error_report_path = self.output_dir / "error_report.html"
